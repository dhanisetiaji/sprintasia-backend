...
type IUserRepository interface {
	...
	Create(user *models.User, register models.RegisterUser) (err error)
...
func (repository *UserRepository) GetUsersWithPaginationAndOrder(pagination scopes.GormPager, order scopes.GormOrderer) (users []models.User, totalCount int64, err error) {
	err = repository.DB().Model(&models.User{}).Scopes(order.ToOrder(models.User{}.TableName(), "id", "id", "created_at", "updated_at")).Count(&totalCount).Scopes(pagination.ToPaginate()).Find(&users).Error
	return
}
...
func (repository *UserRepository) Create(user *models.User, register models.RegisterUser) (err error) {
	hashedPassword, _ := helpers.Hash(register.Password)
